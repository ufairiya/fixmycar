...
class Usertracking
{
   ...
   * @return boolean The result of the tracking action (whether a db record was added or not)
   */
  public function track_this($message="")
  {
    ...
    //get the data
    $input_data = array();
    $input_data['session_id'] = ($this->CI->session->userdata('session_id') !=  NULL ) ? $this->CI->session->userdata('session_id') : time() ;
    ...
    $input_data['user_identifier'] =  get_active_user_id();
    ...
    if ($this->configuration['user_identifier'] !== null && is_array($this->configuration['user_identifier']))
    {
      ...
      if ( !empty( $this->CI->load->$class_type($class_name)))
      {
	...
	if(isset($input_data['user_identifier']) && $input_data['user_identifier'] == FALSE){
		 $input_data['user_identifier'] = 0;
	}
	$result =  TRUE;
